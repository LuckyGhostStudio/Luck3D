...
namespace Lucky
{
    namespace
    {
        void ProcessMesh(const aiMesh* mesh, const aiScene* scene, std::vector<Vertex>& vertices, std::vector<uint32_t>& indices, std::vector<SubMesh>& subMeshes, const ModelLoadOptions& options)
        {
        ...
        void ProcessNode(const aiNode* node, const aiScene* scene, std::vector<Vertex>& vertices, std::vector<uint32_t>& indices, std::vector<SubMesh>& subMeshes, const ModelLoadOptions& options)
        {
            // 处理当前节点的所有 Mesh
    ...
    ModelLoadResult ModelLoader::Load(const std::string& filepath, const ModelLoadOptions& options)
    {
