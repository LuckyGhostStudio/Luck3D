...
**强制花括号（无论几行）**：`if`、`else`、`else if`、`for`、`while`、`do`、`switch` 等控制语句的执行体**必须**使用花括号 `{}` 包裹，即使只有一条语句、甚至该语句可以写在同一行，也**不允许**省略：
...
if (condition) DoSomething();  // 同样禁止：即使写在同一行也不允许
```

**理由**：
1. 避免后续新增语句时忘记加花括号而引入 bug；
2. 与 diff 友好（新增/删除单条语句不会改变缩进结构）；
3. 与项目其余部分保持视觉一致，任何控制语句块的边界都能一眼识别。
...
- 建议行宽不超过 **120 字符**
- **一条语句不因为长度而人为拆成多行**：函数调用、日志/格式化字符串、成员访问链等即使较长也应写在同一行，不要为了对齐好看而在实参、`.` 或运算符处强行换行；只有在**逻辑上有独立子步骤**（如复杂链式表达式的自然断点）或**语句本身跨越了明显的逻辑边界**时才考虑换行
- 换行时续行缩进一级，且不允许出现"实参分两行/多行"的形式

```cpp
// ? 正确：日志与格式化调用即使较长也写在同一行
...
// ? 错误：不要把一条 LF_CORE_ERROR 拆成两行来"对齐参数"
LF_CORE_ERROR("AssetManager::MoveAsset - Failed to rename '{0}' -> '{1}': {2}",
              oldAbs, newAbs, ec.message());
```

**理由**：
1. 项目已有大量单行长参数的日志调用，风格统一；
