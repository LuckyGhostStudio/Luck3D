...
验证：
- Cube 的 6 个面中应显示 4 种不同颜色（共 4 个不同的 Tangent 方向）：
  - 右面 (X+)：Tangent=(0,0,1)，可视化颜色 ≈ (0.5, 0.5, 1.0) 淡蓝色
  - 左面 (X-)：Tangent=(0,0,-1)，可视化颜色 ≈ (0.5, 0.5, 0.0) 暗黄色
  - 上面 (Y+) / 下面 (Y-) / 前面 (Z+)：Tangent=(1,0,0)，可视化颜色 ≈ (1.0, 0.5, 0.5) 粉红色（三个面颜色相同）
  - 后面 (Z-)：Tangent=(-1,0,0)，可视化颜色 ≈ (0.0, 0.5, 0.5) 青色
- Sphere 的 Tangent 方向应沿经线方向平滑变化
- Plane 应显示均匀的粉红色（Tangent 沿 X 轴 = (1,0,0)，可视化颜色 ≈ (1.0, 0.5, 0.5)）
