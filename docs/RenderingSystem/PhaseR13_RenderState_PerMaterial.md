...
> **前置依赖**：  
> - R9 DrawCommand 排序（? 已完成，Phase 1 依赖）  
> - R7 Multi-Pass 渲染管线（? 已完成，RenderCommand 扩展 + Pass 中 GL 调用封装依赖）  
> - R8 选中描边系统（? 已完成，SilhouettePass / OutlineCompositePass 中有裸露 GL 调用需封装）  
> **文档说明**：本文档详细设计 Per-Material 渲染状态系统，使每个材质可以独立控制 CullMode、ZWrite、ZTest、BlendMode、RenderingMode 等 GPU 渲染状态。同时封装 Pass 中的裸露 OpenGL 调用，消除渲染层对 OpenGL 后端的直接耦合。参考 Unity ShaderLab 的渲染状态模型和 Hazel 引擎的 PipelineSpecification 设计。  
> **关联文档**：  
> - `PhaseR7_Multi_Pass_Rendering.md`（Multi-Pass 渲染管线 + PassGroup 分组执行）  
...
- [二、当前项目现状分析](#二当前项目现状分析)
  - [2.1 渲染管线架构（R7 已完成）](#21-渲染管线架构r7-已完成)
  - [2.2 现有渲染状态设置](#22-现有渲染状态设置)
  - [2.3 问题总结](#23-问题总结)
...
- [六、与其他模块的关联](#六与其他模块的关联)
  - [6.1 与 R7 Multi-Pass 渲染管线的关联](#61-与-r7-multi-pass-渲染管线的关联)
  - [6.2 与 R9 DrawCommand 排序的关联](#62-与-r9-drawcommand-排序的关联)
  - [6.3 与 R12 渲染器架构演进的关联](#63-与-r12-渲染器架构演进的关联)
  - [6.4 与 R4 阴影系统的关联](#64-与-r4-阴影系统的关联)
- [七、渐进式实施路线](#七渐进式实施路线)
  - [7.1 Phase 0：立即可做（全局背面剔除）](#71-phase-0立即可做全局背面剔除)
  - [7.2 Phase 1：Per-Material 渲染状态 + Pass GL 调用封装](#72-phase-1per-material-渲染状态--pass-gl-调用封装)
...
> **注意**：本章节基于 R7 Multi-Pass 渲染管线改造完成后的代码状态（2026-04-24）。

### 2.1 渲染管线架构（R7 已完成）

当前渲染管线已完成 R7 改造，采用 `RenderPipeline` + `RenderPass` 架构，4 个 Pass 按分组执行：

```
RenderPipeline
├── "Main" 分组（EndScene() 中执行）
│   ├── OpaquePass        — 不透明物体绘制（无裸露 GL 调用 ?）
│   └── PickingPass       — Entity ID 拾取（有裸露 GL 调用 ?）
│
├── [Gizmo 渲染]          — 在 Main 和 Outline 之间执行
│
└── "Outline" 分组（RenderOutline() 中执行）
    ├── SilhouettePass        — 轮廓掩码渲染（有裸露 GL 调用 ?）
    └── OutlineCompositePass  — 描边合成（有裸露 GL 调用 ?）
```

### 2.2 现有渲染状态设置

#### 全局初始化（`RenderCommand::Init()`）
...
| Pass | 裸露 GL 调用数 | 具体调用 |
|------|--------------|---------|
| **OpaquePass** | 0 | ? 无裸露 GL 调用，完全通过 `RenderCommand` 封装 |
| **PickingPass** | 6 | `glDrawBuffers` ×2、`glDepthMask` ×2、`glDepthFunc` ×2 |
| **SilhouettePass** | 2 | `glDisable(GL_DEPTH_TEST)`、`glEnable(GL_DEPTH_TEST)` |
| **OutlineCompositePass** | 6 | `glDrawBuffers`、`glDisable(GL_DEPTH_TEST)`、`glEnable(GL_BLEND)`、`glBlendFunc`、`glActiveTexture` + `glBindTexture`、`glEnable(GL_DEPTH_TEST)` + `glDisable(GL_BLEND)` |

每个有裸露 GL 调用的 Pass 都需要 `#include <glad/glad.h>`，**直接耦合了 OpenGL 后端**。
...
| **Pass GL 耦合** | ? 3 个 Pass 直接使用裸露 GL 调用 | 耦合 OpenGL 后端，状态恢复易遗漏，语义不清晰 |

**核心问题**：
1. 所有渲染状态都是全局的，没有任何 Per-Material 的渲染状态控制能力
2. 3 个 Pass（PickingPass / SilhouettePass / OutlineCompositePass）直接使用裸露 OpenGL 调用，违反了渲染抽象层的封装原则
...
在 `RenderCommand` 中添加渲染状态控制方法，**消除所有 Pass 中的裸露 GL 调用**：
...
public:
    ...
    /// 用于 PickingPass（只写 EntityID 附件）和 OutlineCompositePass（只写颜色附件）
    /// </summary>
    /// <param name="attachments">颜色附件列表（GL_COLOR_ATTACHMENT0 / GL_NONE 等）</param>
    /// <param name="count">附件数量</param>
    static void SetDrawBuffers(const uint32_t* attachments, uint32_t count);
    
    /// <summary>
    /// 绑定纹理到指定纹理单元
    /// 用于 OutlineCompositePass 绑定 Silhouette 纹理等场景
    /// </summary>
...
{
    glDrawBuffers(count, reinterpret_cast<const GLenum*>(attachments));
}
...
> **重要变更（v2.0）**：R7 改造后，绘制逻辑已从 `Renderer3D::EndScene()` 迁移到 `OpaquePass::Execute()` 中。因此 Per-Material RenderState 的应用也应在 `OpaquePass::Execute()` 中进行。

在 `OpaquePass::Execute()` 的绘制循环中，绘制每个物体前应用其材质的渲染状态。使用**状态跟踪**避免重复设置：

```cpp
// OpaquePass.cpp - Execute() 中的绘制循环
...
#### 5.6.1 Pass 中裸露 GL 调用的封装

R7 改造后，PickingPass / SilhouettePass / OutlineCompositePass 中的裸露 GL 调用也应通过 `RenderCommand` 封装替换：
...
// 封装后（通过 RenderCommand）
uint32_t pickBuffers[] = { GL_NONE, GL_COLOR_ATTACHMENT1 };
RenderCommand::SetDrawBuffers(pickBuffers, 2);
RenderCommand::SetDepthWrite(false);
RenderCommand::SetDepthFunc(DepthCompareFunc::LessEqual);
```

**SilhouettePass 封装前后对比**：
...
// 封装后
uint32_t outlineBuffers[] = { GL_COLOR_ATTACHMENT0, GL_NONE };
RenderCommand::SetDrawBuffers(outlineBuffers, 2);
RenderCommand::SetDepthTest(false);
RenderCommand::SetBlendMode(BlendMode::SrcAlpha_OneMinusSrcAlpha);
RenderCommand::BindTextureUnit(0, silhouetteTextureID);
```

封装后，所有 Pass 都不再需要 `#include <glad/glad.h>`，完全解耦 OpenGL 后端。
...
### 6.1 与 R7 Multi-Pass 渲染管线的关联

R7 改造后，渲染逻辑已从 `Renderer3D::EndScene()` 迁移到独立的 Pass 类中。R13 的设计需要适配这一架构变化：

```
R7 架构影响：
  ├── Per-Material RenderState 应用 → 在 OpaquePass::Execute() 中（而非 EndScene()）
  ├── Pass 中裸露 GL 调用封装 → 通过 RenderCommand 新增方法替换
  │   ├── PickingPass: SetDrawBuffers / SetDepthWrite / SetDepthFunc
  │   ├── SilhouettePass: SetDepthTest
  │   └── OutlineCompositePass: SetDrawBuffers / SetDepthTest / SetBlendMode / BindTextureUnit
  └── Pipeline.ExecuteGroup() → 自动检查 Pass.Enabled，无需手动检查
```

**关键点**：R13 的 `RenderCommand` 扩展不仅服务于 Per-Material 渲染状态，还同时解决了 R7 遗留的 Pass 中裸露 GL 调用问题。这两个目标在同一次实施中完成。

### 6.2 与 R9 DrawCommand 排序的关联
...
### 7.2 Phase 1：Per-Material 渲染状态 + Pass GL 调用封装

> **改动量**：约 300 行新增/修改代码  
> **预计时间**：2-3 天  
> **前置依赖**：R9 DrawCommand 排序（? 已完成）、R7 Multi-Pass 渲染管线（? 已完成）

| 步骤 | 内容 | 涉及文件 |
|------|------|---------|
| ① | 创建 `RenderState.h`（枚举 + 结构体） | 新建文件 |
| ② | `Material` 添加 `RenderState` 成员 | `Material.h` |
| ③ | `RenderCommand` 添加状态切换方法（`SetCullMode` / `SetDepthTest` / `SetDepthWrite` / `SetDepthFunc` / `SetBlendMode` / `SetDrawBuffers` / `BindTextureUnit`） | `RenderCommand.h/cpp` |
| ④ | `RenderCommand::Init()` 修改默认状态（关闭混合、开启剔除） | `RenderCommand.cpp` |
| ⑤ | `OpaquePass::Execute()` 中应用 Per-Material RenderState（带状态跟踪） | `OpaquePass.cpp` |
| ⑥ | **封装 PickingPass 中的裸露 GL 调用**（`glDrawBuffers` / `glDepthMask` / `glDepthFunc` → `RenderCommand::SetXxx()`） | `PickingPass.cpp` |
| ⑦ | **封装 SilhouettePass 中的裸露 GL 调用**（`glDisable/glEnable(GL_DEPTH_TEST)` → `RenderCommand::SetDepthTest()`） | `SilhouettePass.cpp` |
| ⑧ | **封装 OutlineCompositePass 中的裸露 GL 调用**（`glDrawBuffers` / `glDisable` / `glEnable` / `glBlendFunc` / `glActiveTexture` / `glBindTexture` → `RenderCommand::SetXxx()`） | `OutlineCompositePass.cpp` |
| ⑨ | **移除所有 Pass 对 `<glad/glad.h>` 的直接依赖** | `PickingPass.cpp` / `SilhouettePass.cpp` / `OutlineCompositePass.cpp` |
...
| ④ | `OpaquePass::Execute()` 中不透明/透明分区绘制（自动切换 DepthWrite/Blend） | `OpaquePass.cpp` |
...
| `Lucky/Source/Lucky/Renderer/RenderCommand.h` | 修改 | 1 | 添加 `SetCullMode` / `SetDepthTest` / `SetDepthWrite` / `SetDepthFunc` / `SetBlendMode` / `SetDrawBuffers` / `BindTextureUnit` |
| `Lucky/Source/Lucky/Renderer/RenderCommand.cpp` | 修改 | 0+1 | Phase 0: 添加 `glEnable(GL_CULL_FACE)`；Phase 1: 实现状态切换方法 + 修改 Init() |
| `Lucky/Source/Lucky/Renderer/Passes/OpaquePass.cpp` | 修改 | 1 | Execute() 中应用 Per-Material RenderState（带状态跟踪） |
| `Lucky/Source/Lucky/Renderer/Passes/PickingPass.cpp` | 修改 | 1 | 封装裸露 GL 调用 → `RenderCommand::SetXxx()`，移除 `<glad/glad.h>` |
| `Lucky/Source/Lucky/Renderer/Passes/SilhouettePass.cpp` | 修改 | 1 | 封装裸露 GL 调用 → `RenderCommand::SetDepthTest()`，移除 `<glad/glad.h>` |
| `Lucky/Source/Lucky/Renderer/Passes/OutlineCompositePass.cpp` | 修改 | 1 | 封装裸露 GL 调用 → `RenderCommand::SetXxx()`，移除 `<glad/glad.h>` |
| `Luck3DApp/Source/Panels/InspectorPanel.cpp` | 修改 | 1+2 | 材质 Inspector 添加渲染状态编辑控件 |
...
| 决策 | 选择 | 原因 |
|------|------|------|
| RenderState 应用位置 | `OpaquePass::Execute()` 中（而非 `EndScene()`） | R7 改造后绘制逻辑已迁移到 Pass 中，RenderState 应用应跟随绘制逻辑 |
| Pass GL 调用封装时机 | 与 Phase 1 合并实施 | `RenderCommand` 扩展同时服务于 Per-Material 状态和 Pass 内部状态，一次性完成避免重复修改 |
| 新增 `SetDepthTest(bool)` | 独立于 `SetDepthFunc()` | SilhouettePass / OutlineCompositePass 需要完全禁用深度测试（而非修改比较函数），语义更清晰 |
| 新增 `SetDrawBuffers()` | 封装 `glDrawBuffers` | PickingPass / OutlineCompositePass 需要切换写入目标，封装后消除对 `<glad/glad.h>` 的依赖 |
| 新增 `BindTextureUnit()` | 封装 `glActiveTexture` + `glBindTexture` | OutlineCompositePass 需要绑定 Silhouette 纹理，封装后统一纹理绑定接口 |
| 状态跟踪改为局部变量 | `RenderState lastRenderState`（局部） | 避免 `static` 变量在多线程或多场景下的潜在问题，每帧重新初始化更安全 |
