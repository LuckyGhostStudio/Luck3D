...
- **优点**：
  ...
  - 序列化时随 Scene 一起保存/加载
  - LightingPanel 通过 `Ref<Scene>` 引用直接修改，无需额外的中间层（与其他面板模式一致）
...
- 新增 `Ref<Scene> m_Scene` 成员（用于访问 EnvironmentSettings，与其他面板保持一致）
- 新增 `SetScene(const Ref<Scene>& scene)` 方法（与 SceneHierarchyPanel、InspectorPanel 等面板模式一致）
- 新增接受 `const Ref<Scene>&` 参数的构造函数

#### 方案 A：LightingPanel 持有 Ref<Scene> 引用【? 推荐 - 最优】

LightingPanel 通过 `Ref<Scene>` 访问 `EnvironmentSettings`，与 SceneHierarchyPanel、InspectorPanel、SceneViewportPanel 等面板的模式完全一致。

- **优点**：
  - 直接修改 Scene 中的数据，无需中间层
  - 场景切换时只需调用 `SetScene()` 更新引用
  - 序列化时数据自然随 Scene 保存
  - `Ref`（shared_ptr）保证 Scene 在面板持有引用期间不会被销毁，避免悬空指针
  - 与项目中所有其他面板的模式完全一致
- **缺点**：
...
改为：
```cpp
auto lightingPanel = CreateRef<LightingPanel>(m_ActiveScene);
```

并在场景切换时更新：
```cpp
lightingPanel->SetScene(newScene);
```

#### 方案 A：在 EditorLayer 中手动管理 Scene 引用【? 推荐 - 最优】

在 `EditorLayer` 的场景切换逻辑中，手动调用 `lightingPanel->SetScene(newScene)`（与 SceneHierarchyPanel、InspectorPanel 等面板的场景切换模式一致）。

- **优点**：显式控制，逻辑清晰；与项目现有模式完全一致
- **缺点**：需要在每个场景切换点都调用
...
| 方案 | 方式 | 优点 | 缺点 | 推荐度 |
|------|------|------|------|--------|
| **A** | 持有 Ref<Scene> 引用 | 直接修改；与 Inspector 模式一致；生命周期安全 | 依赖 Scene 类 | ? 最优 |
...
| `Luck3DApp/Source/Panels/LightingPanel.h` | 重构：移除 `m_SkyboxMaterial`，新增 `Ref<Scene> m_Scene` 成员和 `SetScene()` 方法 |
...
| 文件 | 新增 include |
|------|-------------|
| `LightingPanel.h` | `#include "Lucky/Scene/Scene.h"`（头文件中直接包含，因为使用了 `Ref<Scene>` 成员） |
